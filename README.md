...
如果批量分析时 Ollama 临时出错、服务断开或模型超时，流云图析会：

1. 保留已经成功的 AI 分析结果。
2. 把当前失败文章记录为失败。
3. 暂停后续批量分析，避免把后面的文章全部刷成失败。
4. 自动保存当前分析结果和未完成队列到本机浏览器存储。

处理方式：

1. 先恢复 Ollama 服务，确认模型能正常响应。
2. 重新打开流云图析。
3. 点击右侧 **继续未完成分析**。
4. 软件会跳过已经成功的文章，只继续未成功/未完成的文章。

如果想重新开始，可以进入“AI 分析结果”页点击“清空分析结果”。
